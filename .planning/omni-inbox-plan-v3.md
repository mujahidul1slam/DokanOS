...
-- ============================================================================
-- LO-2: seed the 'fb/ig' order_source. Verified: the only seed inserts
-- ('online','pos','phone','social','wholesale') — no 'fb/ig' row exists
-- (20260415165743 read in full this session); foundation line 516 is a
-- tolerant SELECT that yields NULL today. Guarded by WHERE NOT EXISTS
-- (order_sources has no name UNIQUE — verified). Also unblocks the
-- foundation's FB selling-point backfill for any future businesses.
-- ============================================================================
...
CREATE INDEX idx_subscriptions_active ON public.notification_subscriptions(channel_account_id, status)
  WHERE status = 'active';
CREATE INDEX idx_subscriptions_topic ON public.notification_subscriptions(channel_account_id, topic, status)
...
BEGIN
  UPDATE public.conversations SET
    ...
    -- Requires a prior customer message (an outbound-only conversation has
    -- nothing to "respond" to — first_response would skew to ~0).
...
(The first_agent_response_at CASE additionally self-guards: it only fires when `last_customer_message_at` is already non-NULL **or** this very UPDATE just set it — the UPDATE statement reads the pre-update row, so for the first-inbound-then-agent sequence the COALESCE window check happens on the next message; the MV filter §11.1 provides the final analytics guard regardless.)
...
-- One-time backfill (batched at migration time; table is small at launch):
-- same resolution logic for existing rows, 10k rows/pass in the migration's
-- DO block, updating only rows where business_id IS NULL.
```

**Scope statement:** audience-builder `min_orders` / LTV / `has_ordered` aggregations (§10.1, §7.4) count **only** orders with non-NULL `business_id` — Woo-linked and inbox-linked orders. Storefront/POS orders that never resolve (no selling point, no store) are **explicitly out of scope for inbox audience analytics** until they gain linkage; the §6.1 inbox order-creation path sets `business_id` directly from the conversation, so inbox-created orders always resolve.
...
// Tag filter (LO-8): array-contains, served by the GIN index (tags @> ARRAY[...])
// supabase.from('conversations').contains('tags', ['vip'])
...
LANGUAGE sql STABLE SECURITY INVOKER SET search_path = public AS $$
  WITH matches AS (
    SELECT m.conversation_id, m.content, c.last_message_at,
           rank() OVER (PARTITION BY m.conversation_id
                        ORDER BY ts_rank_cd(to_tsvector('english', m.content),
                                            websearch_to_tsquery('english', p_query)) DESC) AS r
    FROM public.messages m
    JOIN public.conversations c ON c.id = m.conversation_id
    WHERE c.business_id = p_business_id
      AND m.content_type = 'text'
      AND to_tsvector('english', m.content) @@ websearch_to_tsquery('english', p_query)
    LIMIT 2000            -- hard scan ceiling
  )
...
$$;
-- (RLS applies to both tables under SECURITY INVOKER; the authenticated caller
-- only sees their business's conversations.)
```
...
6. Copy the **App Secret** into edge function secrets: `supabase secrets set META_APP_SECRET=...`. Also set `META_APP_ID`, `META_GRAPH_VERSION` (§5.0), `META_CRON_TOKEN` (vault-held, §5.7)
...
1. **OAuth URL construction** (client or edge function builds it): `https://www.facebook.com/{GRAPH_VERSION}/dialog/oauth?client_id={META_APP_ID}&redirect_uri={APP_URL}/settings/inbox/connect/callback&state={csrf}&scope=pages_show_list,pages_messaging,pages_manage_metadata,business_management`. `state` is a random value stored server-side (signed cookie or a `connect_sessions` row checked at step 3).
...
export async function processOutboxRow(opts: { outboxId: string }) {
  ...
  // 1. Atomic claim (single-writer guarantee)
...
| 3a | No tag, no subscription: `now() < window_expires_at` (NULL = closed) | `blocked_window` |
...
interface Props {
  ...
  catalogScope?: {            // Modification 2 — see below
    productIds?: string[];     // Restrict the self-fetched products/variations to this set
    storeIds?: string[];       // Restrict stores (and products) to the business's selling-point stores
  };
}
...
//     products/products_variations/orders_sources/stores queries gain
//     .in('id', catalogScope.productIds) / .in('id', catalogScope.storeIds)
//     filters when catalogScope is present. The dialog keeps self-fetching
//     (verified: no dataset props exist), but fetches the scoped slice.
...
**Modification 2 — business-scoped catalog (HI-2(d)):** v2's claim that "scoping happens in the wrapper's data fetching" was **false** — the dialog self-fetches its catalog (verified lines 284-321) and accepts no dataset props, so an inbox wrapper could not scope it. The `catalogScope` prop above is the fix: the `OrderLinker` wrapper computes the active business's catalog via `selling_points` (foundation mechanism — `selling_points.business_id` + `woo_store_id`), producing `productIds` (products of the business's stores) and `storeIds` (those stores' ids), and passes them in. The dialog's internal queries then filter — Business A's inbox agents can no longer select Business B's products/stores. The orders page (no `catalogScope`) is unaffected.
...
3. Dialog saves: customer resolution by typed phone (line 782) runs — **this is the merge point** (§7.2). The order insert carries the business_id set by `OrderLinker`? No — the dialog's orders insert does not know the business; `OrderLinker` sets `business_id` on the conversation_orders link, and §2.5.4's resolution trigger derives `orders.business_id` from `selling_point_id`/`store_id` when the agent selects a scoped store. For orders created without a store, `business_id` resolves at the **link** step below (trigger (b) tags via the conversation's business regardless).
...
3. If the resolved-by-phone customer differs from the conversation's alias-created customer:
   ...
   - `OrderLinker` calls `merge_chat_customers(chat_customer_id, canonical_customer_id)` — full SQL:
...
BEGIN
  ...
  -- The conversation being worked gives the business context; the caller
  -- (OrderLinker) passes it via the conversation's customer. Resolve it
  -- from the chat customer's conversations (any one).
  SELECT business_id INTO v_business_id
    FROM public.conversations WHERE customer_id = p_chat_customer_id
  ...
  UPDATE public.notification_subscriptions SET customer_id = p_canonical_customer_id
   WHERE customer_id = p_chat_customer_id;
  UPDATE public.conversation_orders SET customer_id = NULL  -- order links key orders, not customers; orders keep their own customer_id
   WHERE customer_id = p_chat_customer_id;                   -- (defensive; column not used)
...
END;
$$;
REVOKE ALL ON FUNCTION public.merge_chat_customers(uuid, uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.merge_chat_customers(uuid, uuid) TO authenticated;
```
...
- [ ] Migration C: all tables (§2.2), indexes (§2.4), RLS policies in final shape **with HI-3/ME-5/LO-3 relational-consistency clauses** (§2.3), triggers + functions (§2.5 — incl. GREATEST guards, sender assertion, opt-in config triggers, `orders.business_id` resolution + backfill), rate-limit RPC (§5.3), `claim_outbox_batch` RPC (§5.6), token encrypt/decrypt **with extensions-schema pgcrypto** (§13.1), realtime publication (§2.7)
- [ ] Migration D: storage bucket + policies (§2.6)
- [ ] Vault: `inbox_token_key`, `meta_outbox_cron_token`, `meta_bulk_dispatch_token` secrets; cron schedules (outbox sweep §5.6; presence close-out §17.5)
...
-- FTS so ConversationSearch spans the archive (§3.7 RPC already caps results)
...
-- Presence close-out sweep (LO-10):
...
- `supabase/migrations/202609XX03_omni_inbox_schema.sql` (tables, indexes, RLS incl. HI-3/ME-5/LO-3 consistency clauses, triggers/functions, RPCs incl. claim_outbox_batch, vault, pgcrypto-in-extensions, crons)
...
- `supabase/migrations/202609XX06_omni_inbox_orders_business.sql` (orders.business_id + resolution trigger + backfill — ME-9)
