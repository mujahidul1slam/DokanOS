# Storefront P0 Implementation Plan — V4

**Source:** STOREFRONT-AUDIT.md (2026-09-09) · **Revision:** Cycle 4 — resolves all 5 cycle-3 findings (H6; M12; M13; L8; L9) from STOREFRONT-PLAN-V3-CRITIQUE.md, retaining every cycle-1/2/3 resolution · **Executor:** 1 senior full-stack dev · **Estimate:** 44.5–59.5 workdays ≈ **8.9–11.9 weeks** — the exact sequential single-dev sum of the phase efforts (§3; M11 discipline retained)
**Status:** Covers audit P0 items 1–6 only (see §12 Out of scope). V1 (STOREFRONT-PLAN-V1.md), V2 (STOREFRONT-PLAN-V2.md), and V3 (STOREFRONT-PLAN-V3.md) are retained for the audit trail.
...
## 2. Grounding facts (re-verified across cycles 1–4; corrections marked)

These constrain the design. Cycle-4 corrections supersede V3 where they conflict.
...
9. **`storefront_pages` ALREADY EXISTS** (`20260516201928` §5): columns `(id, storefront_id, slug, title, body_md, is_active, created_at, updated_at)`, `UNIQUE(storefront_id, slug)`, public SELECT policy `is_active = true OR has_role(...)`, staff/admin writes. It is a **dead table** — zero app/runtime references (only generated types). Phase 1 EXPANDS this table (add columns, tighten the public policy), it does not CREATE it. A `CREATE TABLE storefront_pages` migration would fail on deploy. **[M12] No inspected migration establishes a BEFORE UPDATE `updated_at` trigger on it — as created, `updated_at` keeps its insert-time value forever. Phase 1 adds the trigger (§4.1); the clobber-detection design (§4.4) depends on it.**
10. **`storefront_collections` + junction `storefront_collection_products` fully exist** with RLS (public read, staff/admin write; junction `product_id` has NO FK; `(collection_id, product_id)` UNIQUE). **[L9] No inspected migration establishes a `position` column on EITHER table.** Phase 2's migration therefore adds `position` to both idempotently (`ADD COLUMN IF NOT EXISTS`) with a deterministic junction backfill before creating the ordering index (§5.2); the verify file records the live pre-flight answer. Phase 2 still needs zero new tables.
...
|---|---|---|---|---|
| 1 | Section-based page builder | #1 | 4–5 wk | expand existing `storefront_pages`; NEW `storefront_page_sections`; **`set_updated_at()` BEFORE UPDATE triggers on both page tables (M12)**; `storefronts.nav`; enum value `storefronts.view` + system-role grant in **two separate migrations (L5)** |
| 2 | Collections wired up | #2 | 3.5–5.5 d | **`position` columns on junction + `storefront_collections` via `ADD COLUMN IF NOT EXISTS` + deterministic junction backfill (L9)** + one index — zero new tables; `src/storefront/lib/routes.ts` hoisted here from Phase 4 (L4) |
...
-- storefront_pages pre-exists (fact 9) with no such trigger established; the
-- DROP IF EXISTS of OUR OWN trigger name + re-CREATE is the standard migration
-- idempotency idiom on a self-owned object (not a policy drop; not an exception
-- to expand-only). If some differently-named trigger already bumps updated_at,
-- ours is harmless (both set now()); the verify file records the answer.
...
- **Save clobber detection (M12 — now actually functional):** section/page saves are last-write-wins *after a detected conflict*; every save sends the row's `updated_at` it was based on and the update matches `WHERE id = ? AND updated_at = <base>` — zero rows updated → conflict toast prompting reload. This works **because** §4.1's `set_updated_at()` BEFORE UPDATE triggers advance `updated_at` on every write to both tables (V3's version was dead as specified: no trigger existed, `updated_at` kept its insert-time value, and the stale-base predicate always matched). The trigger is the mechanism; the WHERE predicate is the check. Detects clobbers; does not resolve them (v1 accepts this, stated in §13 risk 6).
...
### 5.2 DB (migration `storefront_collections_position.sql` + verify — L9)

One migration. **Fact 10 establishes the tables, RLS, and the `(collection_id, product_id)` UNIQUE junction key — but NO `position` column on either table.** `ADD COLUMN IF NOT EXISTS` makes the migration deploy-safe whether or not they pre-exist (the same deploy-blocking shape as V1's `CREATE TABLE storefront_pages`, closed); the executor's pre-flight column check records the live answer in the verify file:

```sql
-- L9: position columns were never established by any inspected migration (fact 10).
ALTER TABLE public.storefront_collection_products
  ADD COLUMN IF NOT EXISTS position integer NOT NULL DEFAULT 0;
ALTER TABLE public.storefront_collections
  ADD COLUMN IF NOT EXISTS position integer NOT NULL DEFAULT 0;

-- Junction backfill: deterministic initial order within each collection (no
-- order column exists — fact 10 — so uuid order is the documented initial
-- state; operators reorder immediately via §5.4). Idempotent: the
-- position <> rn guard makes a re-run a no-op.
WITH ordered AS (
...
`CollectionsTab.tsx`: list (title, slug, active, position, product count), create/edit dialog, delete (cascade clears junction), inline product picker (search like ProductsTab; add/remove/reorder via `position` — grounded by §5.2's columns).

### 5.5 Verification

- Verify migration: `position` columns exist on both tables (the file records whether they pre-existed — L9 pre-flight); junction backfill idempotent (re-run changes nothing); index exists; policies unchanged; dead-table status resolved.
...
- Verify migration: columns/indexes/RPCs/journal column exist; **`uq_orders_order_number` exists (L8)**; **`information_schema.routine_privileges` shows EXECUTE on both RPCs for `service_role` and NO EXECUTE for PUBLIC/anon/authenticated (H6)**; **`pg_policies` shows zero `TO anon` policies on orders, order_items, order_payments, order_timeline, customers, products, stores; `product_variations` has exactly the SELECT-only Phase 3 policy (active-product-gated); `products` has exactly the `20260619164824` SELECT-only policy (H5 — these assertions are only now complete)**.
...
### 14.3 Cycle 4 → V4 (this revision — 5 findings from STOREFRONT-PLAN-V3-CRITIQUE.md)
...
| **M12** | §4.1 DDL gains `set_updated_at()` BEFORE UPDATE triggers on `storefront_page_sections` AND `storefront_pages` (V3 had no bump mechanism — `updated_at` was insert-frozen and clobber detection dead); §4.4 rewritten to name the trigger as the mechanism behind the stale-base WHERE predicate; §4.6 adds pg_trigger presence check + scripted stale-base zero-rows/current-base one-row check + two-tab clobber manual test; §13 risk 6 updated ("real", not aspirational). |
