-- v2 of the Phase-3 harness: T3 needs a product that actually HAS a Woo-linked
-- variation (the first run picked a simple product, UPDATE matched 0 rows, so
-- the variation trigger never fired — harness flaw, not a trigger flaw).
...
BEGIN
  -- Prefer a product WITH a Woo-linked variation so T3 is exercisable.
  SELECT pr.id, pr.stock_quantity, pr.stock_status, pr.woo_product_id, pr.store_id,
         (SELECT count(*) FROM public.product_variations pv
          WHERE pv.product_id = pr.id AND pv.woo_variation_id IS NOT NULL) AS var_count
  INTO p
  FROM public.products pr
  WHERE pr.woo_product_id IS NOT NULL AND pr.store_id IS NOT NULL
  ORDER BY var_count DESC, pr.updated_at DESC
  LIMIT 1;
  IF NOT FOUND THEN
    RETURN jsonb_build_object('error', 'no woo-linked product found');
  END IF;
  v_has_variation := p.var_count > 0;
  ...
  BEGIN
    ...
    -- T3: variation edit enqueues the PARENT (only if variations exist)
  ...
  RETURN jsonb_build_object(
    ...
    't3_variation_enqueues_parent', (NOT v_has_variation) OR ((v_after3 - v_after2) >= 1),
    't3_tested', v_has_variation,
